...
- **Feels like a human call:** if the first spoken audio is slow, ONE short varied
  ...
  lost (if the agent won't interrupt, it runs right after). Rapid repeated
  corrections merge cleanly: the re-run request carries your original words once
  plus each correction once, in spoken order - never nested or duplicated.
  Steering is same-source
